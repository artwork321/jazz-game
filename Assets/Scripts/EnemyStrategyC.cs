...
// never forfeits
public class EnemyStrategyC : EnemyStrategy
{
    // Cheating: viewing player total without cost
    private int GetPlayerTotal() {
        return enemy.gm.player.diceTotal;
    }
    ...
    public override void SelectDie()
    {
        ...
        // Then choose dice to play or forfeit
        int bigCount = 0;
    ...
    private bool TryUsePowerup(int type, Dice[] playedDice, Dice[] playerPlayedDice)
    {
        ...
        if (type == 1 && !isUseReroll && enemy.gm.scoreManager.opponentPts >= 0 && GetPlayerTotal() - 6 <= enemy.diceTotal)
        {
            foreach (Dice die in enemy.playerDice)
            {
                if (die.diceValue < 3)
        ...
        else if (type == 2 && !isUseUpgrade && enemy.gm.scoreManager.opponentPts >= 0 && GetPlayerTotal() - 6 <= enemy.diceTotal)
        {
        ...
        // Only invert col that has player die value > enemy and playerTotal <= enemy total
        else if (type == 3 && !isUseInvert && enemy.gm.scoreManager.opponentPts >= 0 && GetPlayerTotal() - 3 <= enemy.diceTotal)
        {
            for (int i = 0; i < playedDice.Length; i++)
            {
                if (playedDice[i].diceValue < playerPlayedDice[i].diceValue)
